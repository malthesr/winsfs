    ...
    for<'a> Self: Em<'a, N>,
{
    ...
    fn e_step_with_log_likelihood(&self, input: &<Self as Em<N>>::Input) -> (f64, Self) {
        input
            .iter_sites(self.shape())
            .fold(
    ...
    pub fn window_em_step(
        &self,
        input: &<Self as Em<N>>::Input,
        window: &mut Window<N>,
        block_size: usize,
    ) -> Self {
        let sites = input.sites(self.shape());
        ...
        for (i, block) in input.iter_blocks(self.shape(), block_size).enumerate() {
...
impl<const N: usize> Window<N> {
    ...
    pub fn zeros(shape: [usize; N], window_size: usize) -> Self {
        Self(
            std::iter::repeat(Sfs::zeros(shape))
                .take(window_size)
                .collect(),
        )
    }
